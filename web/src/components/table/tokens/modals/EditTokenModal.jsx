...
const EditTokenModal = (props) => {
  ...
  const getInitValues = () => ({
    ...
    ip_rate_limit_count: 0,
    ip_rate_limit_success_count: 0,
    group: '',
    cross_group_retry: false,
    tokenCount: 1,
  });
  const adminRateLimitEnabled =
    statusState?.status?.model_request_rate_limit_enabled ||
    statusState?.status?.model_request_ip_rate_limit_enabled;
  ...
  const submit = async (values) => {
    ...
    if (isEdit) {
      ...
      localInputs.ip_rate_limit_count =
        parseInt(localInputs.ip_rate_limit_count, 10) || 0;
      localInputs.ip_rate_limit_success_count =
        parseInt(localInputs.ip_rate_limit_success_count, 10) || 0;
              ...
              {adminRateLimitEnabled ? (
                <Card className='!rounded-2xl shadow-sm border-0'>
                  ...
                  <Row gutter={12}>
                    ...
                    <Col span={24}>
                      <Form.InputNumber
                        field='ip_rate_limit_count'
                        label={t('令牌+IP 总请求数限制（含失败，0 表示不限）')}
                        min={0}
                        disabled={!values.rate_limit_enabled}
                        placeholder={'0'}
                        style={{ width: '100%' }}
                    ...
                    <Col span={24}>
                      <Form.InputNumber
                        field='ip_rate_limit_success_count'
                        label={t('令牌+IP 成功请求数限制（0 表示不限）')}
                        min={0}
                        disabled={!values.rate_limit_enabled}
                        placeholder={'0'}
                        style={{ width: '100%' }}
